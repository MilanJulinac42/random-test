...
function StatItem({ target, suffix, label, start }: { target: number; suffix: string; label: string; start: boolean }) {
  ...
  return (
    <div className="flex flex-col items-center text-center px-4 py-6">
      <span
        style={{ fontSize: "clamp(56px, 7vw, 64px)", fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 1, color: "#A78BFA" }}
      >
        <span>{value}</span>
        <span>{suffix}</span>
      </span>
      <span
        ...
        style={{ letterSpacing: "0.18em", fontWeight: 500, color: "#999", fontSize: 14 }}
      >
        {label}
      </span>
    </div>
